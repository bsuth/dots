...
local meters = components.panel({
  ...
  layout.hpad(8),
  components.meter({
    icon = models.battery.discharging and beautiful.svg('dashboard/meters/battery-discharging') or beautiful.svg('dashboard/meters/battery-charging'),
    color = 'red',
...
function create_switch_item(config)
  local icon = wibox.widget({
    forced_width = 40,
    forced_height = 40,
    image = config.model.active and config.active_icon or config.inactive_icon,
    widget = wibox.widget.imagebox,
  })

  config.model:connect_signal('update', function()
    icon.image = config.model.active and config.active_icon
