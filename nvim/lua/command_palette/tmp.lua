...
function M.highlight(command_palette)
  ...
  if not head.lazy and command_palette.num_filtered_commands > 0 then
    ...
    vim.hl.range(command_palette.buffer, NAMESPACE_ID, 'FocusedIndex', { focused_row, 0 }, { focused_row, -1 })
  end
  ...
  if has_overflow then
    vim.hl.range(command_palette.buffer, NAMESPACE_ID, 'OverflowEllipses', { 0, 0 }, { 0, -1 })
  end
end
...
function M.refresh(command_palette)
  local head = command_palette.path[#command_palette.path]
  command_palette.filtered_commands = head.callback()
...
function M.open(command_palette)
  ...
  vim.api.nvim_win_set_hl_ns(command_palette.window, NAMESPACE_ID)
end
